...
const Sidebar = ({ miniSidebar, setMiniSidebar}) => {
  ...
  return (
    <aside className={`${miniSidebar ? 'col-1' : 'col-2'} ${mobileSidebar ? 'd-grid' : 'd-grid'} py-3 min-vh-100 bg-whitebg text-blue d-none d-lg-grid align-content-between`}>
      <nav className='d-grid gap-5 ps-2'>
        <ul className='d-flex justify-content-between align-items-center'>
          <li className='overflow-hidden loader'><img src={logo} alt="beta logo" width={miniSidebar ? 50 : 100} height={miniSidebar ? 30 : 60} /></li>
          <li className={`overflow-hidden ${miniSidebar ? 'rotate-180' : 'rotate-90'}`}><img src={back} alt="minimize button" onClick={() => setMiniSidebar(!miniSidebar)} className='rotate-90' /></li>
        </ul>
